...
Set a distinct-layout budget at the same time. Read `intra-film-diversity.md` and count a composition as distinct only when at least three structural axes change. Excluding the final readable hold, at least 70% of major landings should normally use independent spatial skeletons. A subject, copy, card-content, crop, or accent swap inside the same massing does not count. Repeated layouts must be deliberate `A-prime` callbacks with at least three structural changes and a new function.
...
- one coherent motion family and material response;
- at least four compatible observable mechanisms for dense, editorial, collage, playful, or action-led work;
- one dominant direction or spatial tendency;
- four to seven recurring motifs;
- four to eight explicit prohibitions.

Use a dominant-versus-secondary ratio only to balance high-level compatible families. Do not use a 70/30 rule to repeat one panel move, layout scaffold, camera move, or entry-settle-exit sequence. Every shot must draw from the shared dictionary, but adjacent beats must rotate their primary carrier and transformation mechanism unless the reference explicitly supports serial repetition. Coherence comes from shared palette, material, easing, motif, and directional logic; diversity comes from different spatial construction and choreography.
...
Select the typography appearance and animation from the same reference evidence. Do not treat font choice, layout, and motion as separate styling decisions. If type is visible in the reference, transfer its category, width, weight, case, tracking, alignment, outline or shadow, repetition, cropping, distortion, and image relationship. If type is absent, infer conservatively from geometry, material, composition, era, density, and tone. Reject attractive type motion that belongs to a different design culture.
...
- typography appearance and animation are traceable to the same approved reference cluster as the visual and motion system;
...
- replacing the reference with an unrelated style would make the motion plan feel wrong.

If the last test fails, reroute the motion system.
